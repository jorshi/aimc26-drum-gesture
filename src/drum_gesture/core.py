...
class OnsetDetection:
    def __init__(
        self,
        sr: int,
        on_thresh: float = 19.0,
        off_thresh: float = 8.0,
        wait: int = 1323,
        min_db: float = -70.0,
        eps: float = 1e-8,
        fast_env_attack: float = 3.0,
        fast_env_release: float = 383.0,
        slow_env_attack: float = 2205.0,
        slow_env_release: float = 2205.0,
        high_pass_cutoff: float = 2000.0,
    ):
        self.env_fast = EnvelopeFollower(fast_env_attack, fast_env_release)
        self.env_slow = EnvelopeFollower(slow_env_attack, slow_env_release)
        self.high_pass = HighPassFilter(sr, high_pass_cutoff)
    ...
    def _onset_signal(self, x: np.array):
        ...
        return diff, env_fast, env_slow

    def __call__(self, x: np.array):
        assert x.ndim == 2 and x.shape[0] == 1, "Monophone audio only."

        # Calculate envelope
        onset, _, _ = self._onset_signal(x)
    ...
    def update_sample_rate(self, sr: int):
        """
        Update the sample rate of the filter.
        """
        self.high_pass = HighPassFilter(sr, self.high_pass.cutoff)
